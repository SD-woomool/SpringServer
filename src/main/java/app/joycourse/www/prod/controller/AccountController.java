...
public class AccountController {
    ...
    public Response<UserInfo> callback(@RequestParam(value = "code", required = false) String code, @PathVariable("provider") String provider, HttpServletResponse setCookieResponse) throws CustomException {
        ...
        boolean login;
        if (user.isPresent()) {
            jwtToken = jwtService.createToken("id:" + user.get().getId(), jwtConfig.getCookie().getTtlMillis());
            nickname = user.get().getNickname();
            login = true;

        } else {
            jwtToken = jwtService.createToken("email:" + email, jwtConfig.getCookie().getTtlMillis());
            login = false;
        }
        Cookie jwtCookie = new Cookie(jwtConfig.getCookie().getName(), jwtToken); // 여기부터
        jwtCookie.setMaxAge(jwtConfig.getCookie().getTtlMillis());
        jwtCookie.setPath("/");
